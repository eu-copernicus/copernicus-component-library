$(function() {
	...
	//INCLUDE header
	$('<header class="ccl-site-header" role="banner"></header>').load(baseUrl('/_includes/header.html')).prependTo($('.ccl-main-container'));
	...
	//INCLUDE navigation
	$.getJSON(baseUrl("/_includes/json_data/nav.json"+v), function(result){     
		//Build Menu
	...
	$('<div id="themeselector_container"></div>').load(baseUrl('/_includes/theme_selector.html'),
		function() {
			//load configuration JSON file
			$.getJSON(baseUrl("/_includes/json_data/themes.json"+v), function(result){
	...
	$('<div id="howto_container"></div>').load(baseUrl('/_includes/how_to_use.html'),function(){
